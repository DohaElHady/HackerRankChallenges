...
void send_all_acceptable_packages(town* source, int source_office_index, town* target, int target_office_index) {	
    int source_packages=source->offices[source_office_index].packages_count;
    int target_packages=target->offices[target_office_index].packages_count;
    ...
    {
        int package_weight=source->offices[source_office_index].packages[i].weight;
        if(package_weight>=min_weight&&package_weight<=max_weight)
        {
            //target
            target_packages++;
            target->offices[target_office_index].packages=realloc(target->offices                       [target_office_index].packages, sizeof(package) * target_packages);
            ...
            //source
            for (int j=i;j<source_packages;j++)
...
town town_with_most_packages(town* towns, int towns_count) {
    int towns_packages[towns_count];
    int max_packages=0;
    int most_packages_town_id=0;
    for (int i=0;i<towns_count;i++){
        towns_packages[i]=0;
        for (int j=0;j<towns[i].offices_count;j++)
            towns_packages[i]+=towns[i].offices[j].packages_count;
        most_packages_town_id=towns_packages[i]>max_packages? i:most_packages_town_id;
        max_packages=towns_packages[i]>max_packages? towns_packages[i]:max_packages;
    }
    return towns[most_packages_town_id];
}
